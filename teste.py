Okay, aqui está o código refeito, utilizando a estrutura exata do modelo que você forneceu, incluindo a função criarListaComponentes separada e a estrutura inicial da função main com Gn = np.zeros([]) e In = np.zeros(0).
Implementei a lógica da Análise Nodal Modificada (ANM) dentro da função main, após a chamada a criarListaComponentes. Removi todos os comentários (documentação e inline) e todas as chamadas print().
####################################################
...
def main(netlist):
    ...
    componentes = criarListaComponentes(netlist)
    ...
    for elemento in componentes:
        ...
        if comp_type == 'R':
            ...
            b[v_constr_eq_row][0] += valorV
             ...
             A[nIplus][f_source_curr_col] += 1
             A[nIminus][f_source_curr_col] -= 1
             A[nCc_f][f_control_curr_col] -= 1
             A[nCd_f][f_control_curr_col] += 1

             A[f_source_def_row][f_source_curr_col] += 1
             A[f_source_def_row][f_control_curr_col] -= beta
             ...
             A[nVplus][h_source_curr_col] += 1
             A[nVminus][h_source_curr_col] -= 1
             A[nCc_h][h_control_curr_col] -= 1
             A[nCd_h][h_control_curr_col] += 1

             A[h_source_def_row][nVplus] += 1
             A[h_source_def_row][nVminus] -= 1
             A[h_source_def_row][h_control_curr_col] -= rm
